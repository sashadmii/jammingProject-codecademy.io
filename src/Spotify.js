...
export const url = () => {
  ...
  const urlToken = '?response_type=token';
  const urlClientId = `&client_id=${clientId}`;
  const urlScope = `&scope=${scope}`;
  const urlRedirect = `&redirect_uri=${redirectUri}`;
  const urlState = `&state=${state}`;

  const complUrl = `${baseUrl}${urlToken}${urlClientId}${urlScope}${urlRedirect}${urlState}`;
  return complUrl;
};
// request authorization
export const getAuth = async () => {
  const urlToFetch = await url();
